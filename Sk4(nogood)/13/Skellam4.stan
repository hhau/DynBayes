...
parameters {
  ...
  matrix<lower=-lmaxScore, upper=lmaxScore>[T, N-1] u_att_G_raw;
  matrix<lower=-lmaxScore, upper=lmaxScore>[T, N-1] u_att_B_raw;

  matrix<lower=-lmaxScore, upper=lmaxScore>[T, N-1] u_def_raw;
  ...
  real<lower=0, upper=1> phiAttGoal;
  real<lower=0, upper=1> phiAttBehind;
  real<lower=0, upper=1> phiDef;

  # Constrain variances
  real<lower=0.005, upper=5> stdAtt;
  real<lower=0.005, upper=5> stdDef;
...
transformed parameters {
  ...
  attackGoals[1] <- muAttGoals + stdAtt/sqrt(1-phiAttGoal*phiAttGoal) *  (u_att_G_raw[1] * JmatTrans) ;
  attackBehinds[1] <- muAttBehinds + stdAtt/sqrt(1-phiAttBehind*phiAttBehind) * u_att_B_raw[1] * JmatTrans;
  defend[1] <- muDef + stdDef/sqrt(1-phiDef*phiDef) * u_def_raw[1] * JmatTrans;
  ...
  for(k in 1:T-1){
    ...
    if ((k % 23 < 8)  && ( k % 23 > 0)) { # mod by 23 for start of season variance
        timeDelta <- k % 23;
    ...
    attackGoals[k+1] <- phiAttGoal * attackGoals[k] + muAttGoals + decayFactor * stdAtt * u_att_G_raw[k+1] * JmatTrans;
    attackBehinds[k+1] <- phiAttBehind * attackBehinds[k] + muAttGoals + decayFactor * stdAtt * u_att_B_raw[k+1] * JmatTrans;
    defend[k+1] <- phiDef * defend[k] + muDef + decayFactor * stdDef * u_def_raw[k+1] * JmatTrans;
  }
...
model {
  ...
  for(k in 1:T){
    u_att_G_raw[k] ~ multi_normal(Zeros, Smat);
    u_att_B_raw[k] ~ multi_normal(Zeros, Smat);
    u_def_raw[k] ~ multi_normal(Zeros, Smat);
  }

  phiAttGoal ~ beta(100,20);
  phiAttBehind ~ beta(100,20);
  phiDef ~ beta(100,20);
