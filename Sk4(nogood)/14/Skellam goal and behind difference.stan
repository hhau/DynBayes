...
model {
  ...
  for(k in 1:T-1){
    ...
    attackGoals_raw[k+1] ~ multi_normal(phiAttGoal * attackGoals_raw[k] + muAttGoals_raw, decayFactor * stdAtt * Smat);
    attackBehinds_raw[k+1] ~ multi_normal(phiAttBehind * attackBehinds_raw[k] + muAttBehinds_raw, decayFactor * stdAtt * Smat);
