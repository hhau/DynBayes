...
setwd("/Users/hilary/Desktop/Uni/y4/HonThesis/Berwin/Fittingcode/hon-thesis/Skellam2/")
####
...
matplot(1:23,t(param.mat), type = "o", lty=1:6, pch = "+",
        col = 1:5, xlab = "Round number", ylab = "Parameter value", ylim = c(0.0,0.85))
# again, see sigma.atta.vec is getting very close to prior bounds
# a similar story to it's traceplot
legend(x = 1, y = 0.38,c(expression(delta), expression(phi[alpha]), expression(phi[gamma]), expression(phi[beta]),
...
plot(x = predicted.points, y = actual.points) # crikey
...
plot(hexbin(tempdf, xbins = 25), colramp = rf  , xlim = c(-100,100), , xlab = "Predicted point difference", ylab = "Actual point difference difference")
