...
prefit <- stan("Skellam3.stan", data = dat.list, chains = 0)
...
sflist <-
  mclapply(1:4, mc.cores = 4,
           function(i) stan(fit = prefit, data = dat.list,
                            chains = 1, chain_id = i, iter = 5000,
                            refresh = 100, init = "0"))

# # debuging stan fit
#
# fit <- stan(fit = prefit, data = dat.list,
#             chains = 1, iter = 50,
            # refresh = -1, init = "0",)
...
phiAttGoal.vec <- mean(fit.samples$phiAttGoal)
phiAttBehind.vec <- mean(fit.samples$phiAttBehind)
phiDef.vec <- mean(fit.samples$phiDef)
std.attack.vec <- mean(fit.samples$stdAtt)
std.def.vec <- mean(fit.samples$stdDef)
std.mu <- mean(fit.samples$muStd)
...
par.vec <- c(delta, "delta",
             phiAttGoal.vec, "phi.attack.goal",
             phiAttBehind.vec, "phi.attack.behind",
