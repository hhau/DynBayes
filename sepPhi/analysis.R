...
param.mat <- rbind(delta.vec, phi.att.goal.vec, phi.att.behind.vec, phi.def.vec, sigma.att.vec, sigma.def.vec)
matplot(1:23,t(param.mat), type = "o", lty=1:6, pch = "+",
        col = 1:5, xlab = "Round number", ylab = "Parameter value", ylim = c(0.2,0.8))
# again, see sigma.atta.vec is getting very close to prior bounds
# a similar story to it's traceplot
legend(x = 1, y = 0.58,c(expression(delta), expression(phi[alpha]), expression(phi[gamma]), expression(phi[beta]),
                         expression(sigma[a]), expression(sigma[beta])),
       lty = 1:6 ,col = 1:5, cex = 0.8, text.width = 0.9)
